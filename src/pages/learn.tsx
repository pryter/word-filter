          ...
          </div>
          {
            closed ? <div className="flex sm:flex-row flex-col sm:space-x-4 sm:space-y-0 space-y-2">
              ...
              <div className="flex sm:flex-row flex-col sm:space-x-2 sm:space-y-0 space-y-2 justify-center">
                <button className="sm:block hidden rounded-md border border-gray-500 text-gray-700 py-2 px-4 hover:bg-gray-500 hover:text-white active:text-gray-700 active:bg-white" onClick={() => {setProg(prev => (prev - 1 > 0 ? prev - 1 : 0))}}>
                  <ArrowLeftIcon className="w-5 h-5"/>
                </button>
                <button className="flex justify-center sm:hidden rounded-md border border-gray-500 text-gray-700 py-2 px-4 hover:bg-gray-500 hover:text-white active:text-gray-700 active:bg-white" onClick={() => {setProg(prev => (prev - 1 > 0 ? prev - 1 : 0))}}>
                  <div className="flex flex-col relative">
                    <ArrowLeftIcon className="w-4 h-4 absolute top-1 -left-5"/>
                    <span className="w-16">ย้อนกลับ</span>
                  </div>
                </button>
                <button className="rounded-md border border-red-500 text-red-700 py-2 px-4 hover:bg-red-500 hover:text-white active:text-red-700 active:bg-white" onClick={() => {setStatus(false)}}>เป็นคำหยาบ</button>
                <button className="rounded-md border border-green-500 text-green-700 py-2 px-4 hover:bg-green-500 hover:text-white active:text-green-700 active:bg-white" onClick={() => {setStatus(true)}}>ไม่เป็นคำหยาบ</button>
              </div>
            </>
          }
        </div>
